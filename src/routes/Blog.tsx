...
function Post({ route }: PostProps) {
  const postData = LoadPost(route.replace(/\//g, '-'), new Date(route.substring(0, route.lastIndexOf('/'))))

  return (
    <Link to={'/post/' + route} className="post-link">
      <p className="date">{postData.dateString}</p>
      <h2>{postData.title}</h2>
      <Markdown className="summary">{postData.markdown.substring(0, postData.markdown.indexOf('.') + 1).trimStart()}</Markdown>
